        $(document).ready(function() {
          ...
          $(this.document).on("click",".remove_array", function(e){ //user click on remove text
                ...
                var cari_a4 = $(this).parents('.skema').find('.input_array').find('.form-skema')
                $(cari_a1).find(dibalik).remove()
                $(cari_a2).find(dibalik).remove()
                $(cari_a3).find(dibalik).remove()
                $(cari_a4).find(dibalik).remove()
                $(cari_button).remove()
                $(button_add).remove();
                // console.log(cari_a2)
              ...
              <div class="form-skema">
                ...
                <div class="col-xs-4 col-md-3 col-lg-3 skema3">
                  <div data-id="d`+x+`" class="new_form2 d`+x+` wowd`+x+` form-group">
                    <p>
                      <select class="form-control new_select select2 d`+x+`" name="`+search_field+`[value][`+parent_key+`]" style="width: 100%;" id="type">
  ...
  $(document).on('change', '.new_select' ,function() {
     ...
     if(value_select_type == 'array') {
      ...
      $(a).find('.e'+a_cari).append('   <a href="#" class="baten baten-primary skema_new_field '+a_cari+'" title="Add New Array" data-key="'+form_id+'"><i class="fa fa-plus"></i></a>')
  ...
  $(document).on('click', '.skema_new_field', function(){
    ...
    if(x < 11){
      ...
      var inputnya_1 = `
        <div class="col-xs-6 col-md-6 col-lg-6"></div>
        <div class="col-md-2 col-lg-2 skema-baru2 `+ambil_class+`">
        ...
        <div class="col-md-3 col-lg-3 skema-baru3 `+ambil_class+`">
          <div class="form-group">
        ...
        <div class="col-md-1 col-lg-1 skema-baru4 `+ambil_class+`">
          <div class="form-group">
            <p class="hps_child_child `+ambil_class+`">
              <a href="#" class="baten baten-danger remove_array_child remove-button-child `+ambil_class+`"><i class="fa fa-remove"></i></a>
            </p>
          </div>
        </div>
      `;
      ...
      // $('<div class="new_form '+x+' form-group"><input class="get_input form-control" name="'+search_field+'[fill][]" onkeyup="nospaces(this)" type="text" placeholder="New Field array '+ambil_class+'" required/></div>').insertAfter($(this));
      // var skema_baru2 = `
      //   <div class="new_form `+x+` form-group">
      //     <input class="get_input form-control" name="`+search_field+`[fill][]" onkeyup="nospaces(this)" type="text" placeholder="New Field array `+ambil_class+`" required/>
      //   </div>
      // `;
      //  console.log($(this).parents(".skema").find(".form-skema").find('.skema-baru2.'+ambil_class+'').append(skema_baru2)); //add input box

      //  var skema_baru3 = `
      //   <div class="new_form `+x+` form-group">
      //     <input class="get_input form-control" name="`+search_field+`[typedata][]" onkeyup="nospaces(this)" type="text" placeholder="New Field array `+ambil_class+`" required/>
      //   </div>
      //  `;
      //`  console.log($(this).parents(".skema").find(".form-skema").find('.skema-baru3.'+ambil_class+'').append(skema_baru3)); //add input box
    }
  })